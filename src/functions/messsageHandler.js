...
import { wss } from "../servermodule.js";
...
// Socket
import broadcastToLearners from "../socket/broadcastToLearners.js";

// Utilities
import generateUniqueId from "./utilities/generateUniqueId.js";

// Remedial
import generateFlashcard from "./remedial/generateFlashcard.js";

// Quiz
import generateQuiz from "../functions/quiz/generateQuiz.js";

// Probe
import generateAITestQuestion from "./probe/generateAITestQuestion.js";
import processAITestAnswer from "./probe/processAITestAnswer.js";
import generateAITestReport from "./probe/generateAITestReport.js";

// Attention
import processAttentionCheck from "../functions/attention/processAttentionCheck.js";
...
function messsageHandler(ws, data) {
  try {
    const message = data;
    if (message.type === "register") {
      ...
      if (ws.role === "learner") {
        ...
        // Store the answer in the active interest investigation
        wss.clients.forEach(function each(client) {
          if (
            client.role === "educator" &&
            client.readyState === WebSocket.OPEN
          ) {
            // Send the interest answer to the educator
            client.send(
              JSON.stringify({
                type: "interest_answer_received",
                value: message.answer,
                learnerName: message.learnerName,
                studentId: ws.learnerId,
                status: "answered",
                responseTime: 39,
              })
            );
      ...
      console.log(LOG.SOCKET.EVENT.UNRECOGNIZED_EVENT, message.type);
    }
  } catch (error) {
...
export default messsageHandler;
